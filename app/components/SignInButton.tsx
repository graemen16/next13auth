      ...
      {session ? (
        <Menu as="div" className="relative">
          ...
          <Menu.Items className="bg-react dark:text-react absolute right-0 mt-1 flex w-96 origin-top-right flex-col rounded-xl py-6 text-white shadow-lg focus:outline-none dark:bg-white">
            ...
            <Menu.Item>
              {({ active }) => (
                <Link
                  href="/profile"
                  className={clsx(
                    active && "bg-stone-700/50 dark:bt-stone-200",
            ...
            <Menu.Item>
              {(
                { active } // don't know what this active thing is - seems to be if the menu is 'active'
              ) => (
                <button
                  className={clsx(
                  ...
                  onClick={() => signOut()}
                >
                  <ArrowRightOnRectangleIcon className="h-5 w-5 text-stone" />
                  <span>Sign Out</span>
                </button>
              )}
            </Menu.Item>
